"use client";

import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuLabel,
  DropdownMenuTrigger,
  DropdownMenuItem,
} from "@/components/ui/dropdown-menu";
import { ProductColumn } from "./columns";
import { MoreHorizontal } from "lucide-react";
import {
  ArchiveBoxArrowDownIcon,
  GlobeAltIcon,
  PencilSquareIcon,
  TrashIcon,
} from "@heroicons/react/24/outline";
import { useRouter } from "next/navigation";
import axios from "axios";
import toast from "react-hot-toast";

interface CellActionProps {
  row: ProductColumn;
}

export const CellAction: React.FC<CellActionProps> = ({ row }) => {
  const router = useRouter();

  const onArchived = async () => {
    const data = {
      isArchived: !row.isArchived,
    };
    const message = "Archivace úspěšně nastavena";
    try {
      await axios.patch(`/api/product/${row.id}/toggleArchived`, data);
      toast.success(message);
    } catch (error: any) {
      toast.error("Something went wrong.");
    } finally {
      router.refresh();
    }
  };

  const onPublished = async () => {
    const data = {
      isFeatured: !row.isFeatured,
    };
    const message = "Publikace úspěšně nastavena";
    try {
      await axios.patch(`/api/product/${row.id}/toggleFeatured`, data);
      toast.success(message);
    } catch (error: any) {
      toast.error("Something went wrong.");
    } finally {
      router.refresh();
    }
  };

  const onDelete = async () => {
    try {
      await axios.delete(`/api/product/${row.id}`);
      toast.success("Produkt byl smazán.");
    } catch (error) {
      toast.error("Něco se pokazilo.");
    } finally {
      router.refresh();
    }
  };

  return (
    <div className="pt-1">
      <DropdownMenu>
        <DropdownMenuTrigger>
          <div className="mx-2 ">
            <MoreHorizontal className="h-4 w-4" />
            <span className="sr-only">Otevřít nabídku</span>
          </div>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="end">
          <DropdownMenuLabel>Akce: </DropdownMenuLabel>
          <DropdownMenuItem
            className="text-xs font-bold cursor-pointer"
            onClick={() => onPublished()}
          >
            <GlobeAltIcon className="h-5 w-5 mr-2" />
            Změnit publikaci
          </DropdownMenuItem>
          <DropdownMenuItem
            className="text-xs font-bold cursor-pointer"
            onClick={() => onArchived()}
          >
            <ArchiveBoxArrowDownIcon className="h-5 w-5 mr-2" />
            Změnit archivaci
          </DropdownMenuItem>
          <DropdownMenuItem
            className="text-xs font-bold cursor-pointer"
            onClick={() => router.push(`/admin/product/${row.id}`)}
          >
            <PencilSquareIcon className="h-5 w-5 mr-2" />
            Upravit
          </DropdownMenuItem>
          <DropdownMenuItem
            className="text-xs font-bold cursor-pointer"
            onClick={() => onDelete()}
          >
            <TrashIcon className="h-5 w-5 mr-2" />
            Smazat
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
  );
};
